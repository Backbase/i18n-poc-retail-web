...
describe('AuthEventsHandlerService', () => {
  const getInstance = () => {
    ...
    oAuthService.events = events$$.asObservable();
    ...
    const service = new AuthEventsHandlerService(oAuthService, authService);
    const scheduler = new TestScheduler((a, e) => expect(a).toEqual(e));

    return { service, oAuthService, authService, events$$, scheduler };
  };
