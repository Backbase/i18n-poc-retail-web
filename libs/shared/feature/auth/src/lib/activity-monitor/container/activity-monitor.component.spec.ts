...
describe('ActivityMonitorComponent', () => {
  const getInstance = () => {
    ...
    (activityMonitorService as any).events = activityEvents;

    const oAuthService = jasmine.createSpyObj<OAuthService>('OAuthService', ['hasValidAccessToken', 'logOut']);
  ...
  describe('#logoutOnEndEvent', () => {
    ...
    it('should init login flow when end event is emitted and user has invalid access token', async () => {
      const { component, scheduler, activityEvents, oAuthService, authService } = getInstance();
      const { endEvent } = getData();
