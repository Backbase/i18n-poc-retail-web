...
import { defaultNumbersFormat, getConfiguration, getOptions } from './initiate-payment-helpers';
...
const updateAmountOptions = (_: any, control?: AbstractControl) => {
  ...
  const isAmountOptionsZero = amountOptions?.creditCard?.every((option) =>
    option?.amountApiField ? Number(product?.[option.amountApiField]) === 0 : true,
  );
...
const getToAccountForm = (): PaymentFormGroup => ({
...
const getFromAccountForm = (): PaymentFormGroup => ({
